<?php
$_['text_search']                               = 'Пошук';
$_['text_brand']                                = 'Виробник';
$_['text_manufacturer']                         = 'Виробник:';
$_['text_model']                                = 'Код товару:';
$_['text_price']                                = 'Ціна:';
$_['text_reward']                               = 'Бонусні бали:';
$_['text_points']                               = 'Ціна в бонусних балах:';
$_['text_stock']                                = 'Наявність:';
$_['text_instock']                              = 'Є в наявності';
$_['text_tax']                                  = 'Без ПДВ:';
$_['text_discount']                             = ' шт. або більше: ';
$_['text_option']                               = 'Доступні варіанти';
$_['text_minimum']                              = 'Мінімальна кількість для замовлення: %s';
$_['text_write']                                = 'Написати відгук';
$_['text_login']                                = 'Будь ласка <a href="%s">авторизуйтесь</a> або <a href="%s">створити обліковий запис</a> перед тим як написати відгук';
$_['text_no_reviews']                           = 'Немає відгуків про цей товар.';
$_['text_no_review_text']                       = 'Покупець оцінив даний товар, але не написав відгук';
$_['text_no_review_author']                     = 'Кліент';
$_['text_note']                                 = '<span style="color: #FF0000;">Примітка:</span> HTML розмітка не підтримується! Використовуйте звичайний текст.';
$_['text_success']                              = 'Дякуємо за ваш відгук. Він вступив адміністратору для перевірки на спам і незабаром буде опублікований.';
$_['text_related']                              = 'Рекомендовані товари';
$_['text_tags']                                 = '<i class="fa fa-tags"></i>';
$_['text_error']                                = 'Товар не знайдено';
$_['text_payment_recurring']                    = 'Платіжний профіль';
$_['text_trial_description']                    = 'Сума: %s; сплат: %d %s; Кількість платежів: %d, Далі ';
$_['text_payment_description']                  = 'Сума: %s; сплат: %d %s; Кількість платежів: %d ';
$_['text_payment_cancel']                       = 'Сума: %s; сплат: %d %s; Кількість платежів: до скасування';
$_['text_day']                                  = 'день';
$_['text_week']                                 = 'тижня';
$_['text_semi_month']                           = 'півмісяця';
$_['text_month']                                = 'місяць';
$_['text_year']                                 = 'рік';

// Entry                    
$_['entry_qty']                                 = 'Кількість';
$_['entry_name']                                = 'Ім&#39;я';
$_['entry_review']                              = 'Ваш відгук:';
$_['entry_rating']                              = 'Оцінка';
$_['entry_good']                                = 'Добре';
$_['entry_bad']                                 = 'Погано';
$_['entry_captcha']                             = 'Введіть код, зазначений на картинці:';

// Tabs                 
$_['tab_description']                           = 'Огляд';
$_['tab_attribute']                             = 'Характеристики';
$_['tab_review']                                = 'Відгуки';
$_['tab_photos']                                = 'Фото';
$_['button_send_review']                        = 'Відправити відгук';

// Error                    
$_['error_name']                                = 'Ім&#39;я повинно бути від 3 до 25 символів';
$_['error_text']                                = 'Текст відгуку повинен бути від 25 до 1000 символів';
$_['error_rating']                              = 'будь Ласка, виберіть оцінку';
$_['error_product']                             = "Увага: Необхідно обрати товар для додання відгуку";
$_['error_captcha']                             = 'Код, зазначений на картинці, введено невірно';

// flags
// Don't change names, as they correspont flag types in catalog/model/catalog/product.php, in prepareProductList($filter = array())
$_['flag_viewed']                               = '<i class="icon-eye"></i> Переглянуто';
$_['flag_discount']                             = '<i class="icon-percent"></i> Знижка за %d шт';
$_['flag_special']                              = '<i class="icon-tag"></i> Знижка';
$_['flag_bestseller']                           = '<i class="icon-award"></i> Лідер продажів';
$_['flag_bestreviews']                          = '<i class="icon-thumbs-up"></i> Найкращі відгуки';
$_['flag_wishlist']                             = '<i class="icon-heart"></i> Список бажань';
$_['flag_compare']                              = '<i class="icon-compare"></i> Порівняння';
$_['flag_sale']                                 = '<i class="icon-fire"></i> Розпродаж';
$_['flag_new']                                  = '<i class="icon-star"></i> Новинка';
$_['flag_video']                                = '<i class="icon-youtube"></i> Відео';
