<?php
$_['heading_title']    = 'Реферальные ссылки';
$_['text_account']     = 'Кабинет Партнера';
$_['text_description'] = 'Для того, чтобы зарабатывать по партнерской программе с покупок приводимых Вами покупателей, необходимо добавить в ссылку реферальный код. Воспользуйтесь генератором ссылок для сайта %s.';
$_['entry_code']       = 'Ваш Реферальный Код';
$_['entry_generator']  = 'Создать реферальную ссылку';
$_['entry_link']       = 'Реферальная ссылка';
$_['help_generator']  = 'Введите название товара, на который нужно создать ссылку';