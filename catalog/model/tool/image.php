...
class ModelToolImage extends Model {
	public function resize($filename, $width, $height) {
		if (!is_file(DIR_IMAGE . $filename) || substr(str_replace('\\', '/', realpath(DIR_IMAGE . $filename)), 0, strlen(DIR_IMAGE)) != str_replace('\\', '/', DIR_IMAGE)) {
			// echo('file '.DIR_IMAGE . $filename.' not found');
			// return;
		...
		if ($this->request->server['HTTPS']) {
			return $this->config->get('config_ssl') . 'image/' . ((isset($webp_image) && $webp_image) ? $webp_image : $image_new);
		} else {
			return $this->config->get('config_url') . 'image/' . ((isset($webp_image) && $webp_image) ? $webp_image : $image_new);
		}
	}
}
