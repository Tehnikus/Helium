...
class ModelBlogArticle extends Model {
		...
		if (isset($articles) && !empty($articles)) {
			...
			foreach ($articles as $key => $article) {
				...
				if (isset($article['image']) && $article['image'] !== '') {
					...
					$articles[$key]['image'] = $this->model_tool_image->resize('no_image.webp', $this->config->get('configblog_article_miniature_image_width'), $this->config->get('configblog_article_miniature_image_height'));
				}
			}
		}
		return $articles;
			...
			$sql = "
				SELECT 
					...
					(SELECT 
						AVG(rating) 
					AS total 
					FROM " . DB_PREFIX . "review_article r1 
					WHERE r1.article_id = p.article_id 
						AND r1.status = '1' 
					GROUP BY r1.article_id) AS rating, 
					(SELECT 
						count(*) 
					FROM " . DB_PREFIX . "review_article r1 
					WHERE r1.article_id = p.article_id 
						AND r1.status = '1' 
					GROUP BY r1.article_id) AS reviews

				FROM " . DB_PREFIX . "article p 
				LEFT JOIN " . DB_PREFIX . "article_description pd 
				ON (p.article_id = pd.article_id) 
				LEFT JOIN " . DB_PREFIX . "article_to_store p2s 
				ON (p.article_id = p2s.article_id)
			"; 
	...
	public function getArticleRelated($article_id) {
		...
		$query = $this->db->query("
			SELECT 
				* 
			FROM " . DB_PREFIX . "article_related pr 
			LEFT JOIN " . DB_PREFIX . "article p ON (pr.related_id = p.article_id) 
			LEFT JOIN " . DB_PREFIX . "article_to_store p2s ON (p.article_id = p2s.article_id) 
			WHERE pr.article_id = '" . (int)$article_id . "' AND p.status = '1' 
				AND p.date_available <= NOW() 
				AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'
		");
	...
	public function getArticleRelatedProduct($article_id) {
		...
		$query = $this->db->query("
			SELECT 
				* 
			FROM " . DB_PREFIX . "article_related_product np 
