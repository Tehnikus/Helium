...
class ModelCatalogCategory extends Model {
	public function addCategory($data) {
		...
		foreach ($data['category_description'] as $language_id => $value) {
			if (utf8_strlen($value['meta_title']) < 1) {
				$value['meta_title'] = $value['name'];
			}
			if (utf8_strlen($value['meta_h1']) < 1) {
				$value['meta_h1'] = $value['name'];
			}
			$this->db->query("INSERT INTO " . DB_PREFIX . "category_description SET category_id = '" . (int)$category_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape(htmlspecialchars_decode($value['description'])) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_h1 = '" . $this->db->escape($value['meta_h1']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
		}
	...
	public function getCategories($data = array()) {
		$sql = "
			...
			-- LEFT JOIN " . DB_PREFIX ."product_to_category p2c ON (p2c.category_id = c1.category_id)
	...
	public function getCategoriesByParentId($parent_id = 0, $data = array()) {
		$sql= "
			SELECT 
				*, 
				(SELECT COUNT(parent_id) FROM " . DB_PREFIX . "category WHERE parent_id = c.category_id) AS children,
				(SELECT COUNT(p2c.product_id) FROM " . DB_PREFIX . "product_to_category p2c WHERE p2c.category_id = c.category_id) AS product_count,
				-- Длина описания категории
				CHAR_LENGTH(cd.description) as description_length,
				-- Подсчет h1 с учетом колонки meta_h1 в базе данных
				IF(
					cd.meta_h1 IS NOT NULL, 
					1 + CEIL((CHAR_LENGTH(cd.description) - CHAR_LENGTH(replace(cd.description ,'<h1','')))/3), 
					CEIL((CHAR_LENGTH(cd.description) - CHAR_LENGTH(replace(cd.description ,'<h1','')))/3)
				) as h1_count,
				-- Подсчет h2, h3 и ссылок в тексте
				-- Работает так: (длина описания) - (длина описания без искомого тега) / колво символов в теге
				CEIL((CHAR_LENGTH(cd.description) - CHAR_LENGTH(replace(cd.description ,'<h2','')))/3) as h2_count,
				CEIL((CHAR_LENGTH(cd.description) - CHAR_LENGTH(replace(cd.description ,'<h3','')))/3) as h3_count,
				CEIL((CHAR_LENGTH(cd.description) - CHAR_LENGTH(replace(cd.description ,'href=','')))/5) as ahref_count
		...
		if (isset($data['order']) && in_array($data['order'], $order_data)) {
			$sql .= " ". $data['order'];
		}
		$query = $this->db->query($sql);
		// print_r($query->rows);
