	...
	public function addManufacturer($data) {
		...
		foreach ($data['manufacturer_description'] as $language_id => $value) {
			if (utf8_strlen($value['meta_title']) < 1) {
				$value['meta_title'] =$data['name'];
			}
			if (utf8_strlen($value['meta_h1']) < 1) {
				$value['meta_h1'] = $data['name'];
			}
			$this->db->query("INSERT INTO " . DB_PREFIX . "manufacturer_description SET manufacturer_id = '" . (int)$manufacturer_id . "', language_id = '" . (int)$language_id . "', description = '" . $this->db->escape(htmlspecialchars_decode($value['description'])) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_h1 = '" . $this->db->escape($value['meta_h1']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
		}
