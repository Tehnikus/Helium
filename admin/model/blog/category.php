...
class ModelBlogCategory extends Model {
	public function addCategory($data) {
		...
		foreach ($data['category_description'] as $language_id => $value) {
			...
			$this->db->query("
				INSERT INTO " . DB_PREFIX . "blog_category_description 
				SET 
	...
	public function editCategory($blog_category_id, $data) {
		...
		foreach ($data['category_description'] as $language_id => $value) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "blog_category_description SET blog_category_id = '" . (int)$blog_category_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape(htmlspecialchars_decode($value['description'])) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_h1 = '" . $this->db->escape($value['meta_h1']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
		}
		// MySQL Hierarchical Data Closure Table Pattern
	...
	public function getAllCategories() {
		$query = $this->db->query("
			...
			LEFT JOIN " . DB_PREFIX . "blog_category_to_store c2s 
				ON (c.blog_category_id = c2s.blog_category_id) 
