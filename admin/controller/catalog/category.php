...
class ControllerCatalogCategory extends Controller {
	...
	protected function getList() {
		...
		$data['sort_description_length'] = $this->url->link('catalog/category', 'user_token=' . $this->session->data['user_token'] . '&sort=description_length' . $url, true);
		$data['sort_h1_count'] 			 = $this->url->link('catalog/category', 'user_token=' . $this->session->data['user_token'] . '&sort=h1_count' . $url, true);
		$data['sort_h2_count'] 			 = $this->url->link('catalog/category', 'user_token=' . $this->session->data['user_token'] . '&sort=h2_count' . $url, true);
		$data['sort_h3_count'] 			 = $this->url->link('catalog/category', 'user_token=' . $this->session->data['user_token'] . '&sort=h3_count' . $url, true);
		$data['sort_ahref_count'] 		 = $this->url->link('catalog/category', 'user_token=' . $this->session->data['user_token'] . '&sort=ahref_count' . $url, true);
		...
		$data['sort_sort_order'] 		 = $this->url->link('catalog/category', 'user_token=' . $this->session->data['user_token'] . '&sort=c.sort_order' . $url, true);
		$data['sort_noindex'] 			 = $this->url->link('catalog/category', 'user_token=' . $this->session->data['user_token'] . '&sort=c.noindex' . $url, true);
	...
	protected function getForm() {
		...
		if (isset($this->request->post['image']) && is_file(DIR_IMAGE . $this->request->post['image'])) {
			$data['thumb'] = $this->model_tool_image->resize($this->request->post['image'], $this->config->get('image_category_width'), $this->config->get('image_category_height'));
		...
		$data['placeholder'] = $this->model_tool_image->resize('no_image.webp', $this->config->get('image_category_width'), $this->config->get('image_category_height'));
	...
	private function getCategories($parent_id, $parent_path = '', $indent = '') {
		...
		$results = $this->model_catalog_category->getCategoriesByParentId($parent_id, $sort_order_limit);
		foreach ($results as $result) {
			...
			// Форматируем описание категории
			if (mb_strlen($result['description']) > 0) {
				$description = html_entity_decode($result['description']);
				// Заменяем заголовки на пробелы, чтобы все было красиво
				// Простой strip_tags не оставляет пробелов после h1-h6
				$re = '/<h[1-6](.*?)>(.*?)<\/h[1-6]>/m';
				$subst = ' $2 ';
				$description = preg_replace($re, $subst, $description);
				// Убираем остальные теги
				$description = strip_tags($description);
				// Дилниа текста в символах
				$length =  mb_strlen($description);
				// Количество слов, разделенных пробелами
				$word_count = count(preg_split('/\s+/', $description));
				// Вывод краткого описания
				$description = mb_substr($description, 0, 100);
				// Данные о тексте - длина в символах, кол-во слов
				$text_size = sprintf($this->language->get('description_length'), $word_count, $length);
				$description .= "...".$text_size;				
			} else {
				$description = '<b style="color:red; display: block; text-align: center;">'.$this->language->get('no_description').'</b>';
			}
			...
			$output[$result['category_id']] = array(
				...
				'h1_count'		=> ($result['h1_count'] == 1) ? '<b style="color:green">'.$result['h1_count'].'</b>' : '<b style="color:red">'.$result['h1_count'].'</b>',
				'h2_count'		=> ($result['h2_count'] != 0) ? '<b style="color:green">'.$result['h2_count'].'</b>' : '<b style="color:red">'.$result['h2_count'].'</b>',
				'h3_count'		=> ($result['h3_count'] != 0) ? '<b style="color:green">'.$result['h3_count'].'</b>' : '<b style="color:red">'.$result['h3_count'].'</b>',
				'ahref_count'	=> '<b>'.$result['ahref_count'].'</b>',
